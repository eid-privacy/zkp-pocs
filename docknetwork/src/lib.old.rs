...
use ark_ec::{AffineRepr, CurveGroup, pairing::Pairing, short_weierstrass::Affine};
...
mod c03_holder_binding;

#[derive(Debug)]
pub struct Common {
    setup: PublicSetup,
    kp_issuer: KeypairG2<Bls12_381>,
    kp_holder: P256Keypair,
...
impl VerifiedCredential {
    ...
    pub fn new(
        ...
        key_secr: SecretKey<Bls12<Config>>,
    ) -> Self {
        let pk_x = from_base_field_to_scalar_field::<Fq, BlsFr>(key_pub.x().unwrap());
        let pk_y = from_base_field_to_scalar_field::<Fq, BlsFr>(key_pub.y().unwrap());
        let mut generator = names::Generator::default();
...
mod test {
    ...
    #[test]
    fn proof_predicate_age() -> Result<(), Box<dyn Error>> {
        // Set up parties
        let setup = PublicSetup::new();
        let mut issuer = Issuer::new(setup.clone());
        let mut holder = MobilePhone::new(setup.clone());
